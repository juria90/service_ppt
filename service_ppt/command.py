"""Command module for service_ppt.

This module contains Command classes that generate PowerPoint slides for church services.
It supports operations like generating Bible verse slides, inserting lyrics, finding and
replacing text, duplicating slides, and exporting slides as images.
"""
...
class OpenFile(Command):
    ...
    def execute(self, cm, prs):
        ...
        if self.notes_filename:
            ...
            notes = ""
            with open(self.notes_filename, encoding="utf-8") as f:
                notes = f.read()
...
class SaveFiles(Command):
    ...
    def execute(self, cm, prs):
        ...
        if self.verses_filename:
            ...
            try:
                with open(verses_filename, "w", encoding="utf-8") as f:
...
class GenerateBibleVerse(Command):
    ...
    def split_verses(self, verses: str) -> list[str]:
